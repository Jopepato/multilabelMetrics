...
class multilabelMetricsTest(unittest.TestCase):
    ...
    def test_exampleBasedAccuracy(self):
        ...
        params = readParams('results.txt')
        classifier = MLkNN(k=10)
        MLkNN.fit(Xtrain, ytrain)
        predictions = MLkNN.predict(Xtest)

        self.assertAlmostEqual(params['Accuracy'], eb_accuracy(ytest, predictions))

    def test_exampleBasedPrecision(self):
        Xtrain, ytrain = readDataFromFile('/tests/emotions0.train')
        Xtest, ytest = readDataFromFile('/tests/emotions0.gen')
        params = readParams('/tests/results.txt')
        classifier = MLkNN(k=10)
        MLkNN.fit(Xtrain, ytrain)
        predictions = MLkNN.predict(Xtest)
    ...
    def test_aucMicro(self):
        ...
        classifier = MLkNN(k=10)
        MLkNN.fit(Xtrain, ytrain)
        probabilities = MLkNN.predict_proba(Xtest)
